...
def format_review_markdown(state: dict[str, Any]) -> str:
    ...
    if role_pathways:
        lines.append("")
        lines.append("## Role-Based Testing")
        lines.append("")
        for pathway in role_pathways:
            lines.append(f"### {pathway.role.title()}")
            lines.append(f"_{pathway.description}_")
            lines.append("")
            lines.append(f"**Login:** {pathway.login_hint}")
            lines.append("")
            if pathway.accessible_routes:
                lines.append("**Accessible Routes (verify access):**")
                for route in pathway.accessible_routes:
                    lines.append(f"- [ ] `{route}`")
                lines.append("")
            if pathway.restricted_routes:
                lines.append("**Restricted Routes (verify denied):**")
                for route in pathway.restricted_routes:
                    lines.append(f"- [ ] `{route}` — should be denied")
                lines.append("")
            if pathway.steps:
                lines.append("**Test Steps:**")
                for step in pathway.steps:
                    url_part = f" (`{step.url}`)" if step.url else ""
                    lines.append(f"- [ ] **[{step.priority.upper()}]** {step.action}{url_part}")
